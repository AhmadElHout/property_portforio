...
const AvgTimeToCloseAge = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('http://localhost:3000/api/analytics/time-to-close/age', {
            ...
            if (response.ok) {
                ...
                showToast('Failed to load age data', 'error');
            }
        } catch (error) {
    ...
    return (
        <div className="card">
            <h4 style={{ marginBottom: 'var(--space-md)' }}>Avg Time-to-Close by Property Age</h4>

            {loading ? (
                <div style={{ textAlign: 'center', padding: 'var(--space-xl)' }}>Loading...</div>
            ) : data.length === 0 ? (
                <div style={{ textAlign: 'center', padding: 'var(--space-xl)', color: 'var(--text-secondary)' }}>
                    No closed properties with construction year data
                </div>
            ) : (
                <>
                    {/* Table */}
                    <table style={{ width: '100%', marginBottom: 'var(--space-md)', fontSize: '0.875rem' }}>
                        <thead>
                            <tr style={{ borderBottom: '2px solid var(--border)', textAlign: 'left' }}>
                                <th style={{ padding: 'var(--space-sm)' }}>Age Range</th>
...
export default AvgTimeToCloseAge;
